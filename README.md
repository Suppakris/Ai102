...
- **Slide audit agents.** `POST /api/presentation/audit-slide` runs a two-agent quality pipeline over a generated slide (per `ai-slide-auditor-implementation-plan.md`): an auditor agent lists every claim and marks it SUPPORTED / UNSUPPORTED / INSUFFICIENT_CONTEXT against the outline + source context — asking questions instead of guessing — then scores 0–100 on a weighted rubric (accuracy 40, outline alignment 30, clarity 20, design 10); below-threshold slides (default 80) are regenerated from the audit's revision notes in a bounded loop; an independent reviewer agent then double-checks the audit (confirm / revise_up / revise_down + adjusted score) and returns recommendations. Requests with too little context are refused instead of audited blind, and rounds are persisted to the `SlideAudit` table when a `slideId` is supplied (`src/ai/agents/verification/`).
