...
async def convert_pdf_to_slides(file: UploadFile = File(...)):
    ...
    Pipeline: Upload → Parse → Groq AI → Build PPTX → Return download URL
    """
    ...
    try:
        ...
        if not pages:
            raise HTTPException(status_code=422, detail="No readable text found in PDF. It may be image-only.")
        ...
        if not slides_data:
            raise HTTPException(status_code=422, detail="AI could not extract meaningful content from this PDF.")

        # ── Groq: generate presentation title ──────────────────
...
def download_pptx(job_id: str):
    """Direct download endpoint for generated .pptx files."""
