...
// Deliberately anonymous: real GitHub OAuth is restored (see
// src/server/auth.ts), so an E2E suite that needs to actually sign in would
// need a test GitHub account wired through CI secrets — out of scope for
...
test("sign-in page is reachable and offers GitHub sign-in", async ({ page }) => {
